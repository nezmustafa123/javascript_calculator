...
document.querySelectorAll('#divide,#multiply,#add,#subtract').forEach(operator => operator.onclick = e => {
//  console.log(display.innerText)
	const operator = e.target.dataset.value;
	
  if([...'+*/-'].includes(display.innerText.slice(-1)) 
	// && operator!=='-'
	) { /// <- checks the last character in the display
	  display.innerText = display.innerText.slice(0,-1) + operator // <- removes the last char before adding the new operator
	  //display.innerText += e.target.dataset.value
} else {
	display.innerText += operator // <- just adds the new operator
}
});
